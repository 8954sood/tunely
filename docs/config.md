...
| `allow_existing_subdomain_resources` | bool | 아니오 | 기존 DNS/route가 이미 있으면 takeover 허용 (`false`면 충돌 시 실패) |

참고:

- `caddy_upstream` 미지정 시 `listen` 값을 사용합니다.
- 동적 모드 DNS 레코드는 Cloudflare에서 `proxied=false`(DNS only)로 생성됩니다.
- `allow_existing_subdomain_resources` 기본값은 `false`입니다.
...
`--allow-existing-subdomain-resources`도 값 없이 사용하면 `true`로 처리됩니다.
...
기존 DNS/route가 이미 있을 때:

- 기본(`allow_existing_subdomain_resources: false`): provisioning 실패
- 활성화 시(`true`): 기존 리소스를 takeover(덮어쓰기) 허용
